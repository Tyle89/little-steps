<!--
  At-a-glance home screen: latest milestone, latest mood, latest growth
  measurement. Deliberately shows only summaries with links to the full
  pages (Child, Mood, Stats) rather than duplicating their content.
-->
<template>
  <div class="dashboard">
    <!-- Header -->
...
// Re-exposing only the fields this view actually needs, as a computed,
// keeps the template simple and avoids importing/destructuring every
// single field of the child store here.
...
// The single most recent growth measurement, regardless of whether it was
// entered during onboarding (AddFirstChildView) or later from the Stats
// page - both write to the same `mesures` array, so this dashboard always
// reflects the true latest data point instead of a separately-tracked
// "current weight" field that could drift out of sync.
