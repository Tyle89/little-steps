<!--
  Growth tracking page: two separate line charts (weight, height), each
  plotting the child's own measurements against official WHO reference
  percentile curves for their age and sex.
-->
<template>
...
// WHO growth data is split by sex ('garcon' / 'fille' in
// whoGrowthStandards.js). Anything else (unset, "Non défini") means the
// reference curves simply cannot be shown - there is no unisex WHO table.
...
// Converts a measurement's calendar date into the child's age in months
// at that date. This is required because the WHO reference data (and
// the chart's x-axis) is indexed by age, not by calendar date - two
// children measured on different dates need their points aligned by age
// to be comparable against the same reference curve.
...
// Formats an age-in-months x-axis tick as e.g. "6m" or "2a3m".
...
// Builds one Chart.js line chart (weight or height) with the child's own
// data plus, when the sex is known, the WHO P3/P50/P97 reference curves
// shaded as a "normal range" band.
const buildChart = (canvasRef, indicator, unitLabel, childColor) => {
  ...
  if (genreConnu.value) {
    ...
    // Three extra datasets: the 97th percentile line (filled down to the
    // 3rd percentile line right after it in the array, via `fill: '+1'`,
    // producing the shaded "normal range" band), the median as a dashed
    // reference line, and the 3rd percentile line itself. `order`
    // controls draw order so the child's own line stays on top.
    datasets.push(
      {
  ...
  return new Chart(canvasRef.value, {
    ...
    options: {
      responsive: true,
      // IMPORTANT: without this, the chart computes a height of 0px and
      // renders nothing, unless its container has an explicit CSS height
      // (see .chart-container below) - a classic Chart.js pitfall.
      maintainAspectRatio: false,
      // Each dataset supplies its own {x, y} points instead of sharing a
      // single `labels` array, since the WHO curves and the child's own
      // measurements don't share the same x values (ages).
      parsing: false,
      scales: {
        x: {
          type: 'linear', // numeric age axis, not category labels
          min: 0,
          max: 60,
      ...
      plugins: {
        legend: {
          // Hides the 3rd/97th percentile lines from the legend - they're
          // visually obvious as the edges of the shaded band, and listing
          // them by name would just add clutter.
...
const createCharts = () => {
  ...
  // reusing the same <canvas> without doing this throws a "Canvas is
...
// Rebuilds both charts whenever the underlying data changes: new/edited
// measurements, or a genre/dateNaissance change (which affects whether
// WHO curves show and how ages are computed).
